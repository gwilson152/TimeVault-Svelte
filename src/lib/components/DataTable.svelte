...
<div class="container-glass overflow-hidden">
	{#if title || searchable || customHeader}
		<div
			class="flex flex-col items-start justify-between gap-3 border-b border-gray-700/20 p-4 md:flex-row md:items-center"
		>
			{#if title}
	...
	<div class="overflow-x-auto">
		...
		{:else if data.length === 0 || processedData.length === 0}
			<div class="py-8 text-center" in:fade={{ duration: 200 }}>
				...
				<thead class="data-table-header">
					<tr>
						...
						{#each columns as column}
							...
							>
								<div
									class="flex items-center {column.align === 'right'
										? 'justify-end'
										: column.align === 'center'
											? 'justify-center'
											: ''}"
					...
					{#each paginatedData as row}
						{#if row._dataTableVisible !== false}
							<tr
								...
								onclick={() => dispatch('rowClick', { row, id: row._dataTableId })}
							>
								{#if nestedKey}
										...
										<div style={getNestedIndentStyle(row._dataTableLevel)} class="flex">
											{#if row._dataTableHasChildren}
												<button
													...
													onclick={() => toggleRowExpansion(row._dataTableId)}
												>
													<Icon
														src={expandedRows.has(row._dataTableId)
															? ChevronDown
															: ChevronRightIcon}
														class="h-4 w-4"
													/>
								...
								{#if selectable}
									<td class="w-10" onclick={(e) => e.stopPropagation()}>
										<input
											...
											onchange={() => handleRowSelect(row._dataTableId)}
										/>
									</td>
								{/if}
								...
								{#each columns as column}
									<td
										class={typeof column.cellClass === 'function'
											? column.cellClass(getValue(row, column.key), row)
											: column.cellClass || ''}
										class:right-aligned={column.align === 'right'}
										class:text-center={column.align === 'center'}
									>
	...
	{#if pageable && totalPages > 1}
		<div
			class="flex flex-col items-center justify-between gap-3 border-t border-gray-700/20 p-4 sm:flex-row"
		>
			<div class="text-sm text-gray-400">
				Showing {Math.min((currentPage - 1) * pageSize + 1, processedData.length)} to {Math.min(
			...
			<div class="flex items-center gap-2">
				<div class="hidden items-center sm:flex">
					<select
						class="form-select mr-2 px-2 py-1 text-sm"
				...
				<div class="flex">
					{#if totalPages <= 7}
						{#each Array(totalPages) as _, i}
							<button
								class="table-action-button {currentPage === i + 1
					...
					{:else}
						...
						{#each Array(5) as _, i}
							{#if currentPage > 3 && currentPage < totalPages - 2}
								{#if i + currentPage - 2 > 1 && i + currentPage - 2 < totalPages}
									<button
										class="table-action-button {currentPage === i + currentPage - 2
											? 'bg-blue-500/20 text-blue-300'
											: ''}"
										onclick={() => handlePageChange(i + currentPage - 2)}
									>
										{i + currentPage - 2}
									</button>
								{/if}
							{:else if currentPage <= 3}
								{#if i + 2 < totalPages}
									<button
										class="table-action-button {currentPage === i + 2
											? 'bg-blue-500/20 text-blue-300'
											: ''}"
								...
								{/if}
							{:else if i + totalPages - 5 > 1}
								<button
									class="table-action-button {currentPage === i + totalPages - 5
										? 'bg-blue-500/20 text-blue-300'
										: ''}"
									onclick={() => handlePageChange(i + totalPages - 5)}
								>
									{i + totalPages - 5}
								</button>
							{/if}
						...
						<!-- Last page -->
						<button
							class="table-action-button {currentPage === totalPages
								? 'bg-blue-500/20 text-blue-300'
								: ''}"
	...
	{#if customFooter || $$slots.footer}
		<div class="border-t border-gray-700/20 p-4">
