...
- [Completed and Pending Tasks](./development-todo.md) — Always keep updated and timestamp each task upon completion.
...
When displaying client hierarchies in dropdowns:
1. Use proper spacing and visual hierarchy
2. Add client type indicators using emojis or icons (🏢 Business, 📁 Container, 👤 Individual)
3. Use indentation to show parent-child relationships
4. Sort clients so parents appear before their children
5. Add clear helper text explaining available options
...
>
  ...
  {#each hierarchicalClients as { client, level }}
...
Tables in TimeVault follow a consistent design pattern using the glass-morphism approach with proper contrast for accessibility.

### Table Classes
- `data-table`: Base class for all tables (sets width and border-collapse)
- `data-table-header`: Header row styling with dark background for contrast
- `data-table-row`: Body row styling with hover effect
- `data-table-footer`: Footer row styling for totals/summaries

### Header Styling
Header cells use high contrast to ensure readability:
```css
.data-table-header {
  background-color: rgba(17, 24, 39, 0.7); /* Dark background with opacity */
  color: rgb(249, 250, 251); /* Light text for contrast */
  font-size: 0.75rem;
...
### Table Rows
Rows have subtle hover effects and borders:
```css
.data-table-row {
...
### Alignment Options
For numeric data and actions, use right alignment:
```css
.data-table-header th.right-aligned,
.data-table-row td.right-aligned {
  text-align: right;
}
```
  ...
  <tbody>
    {#each items as item}
      <tr class="data-table-row">
        <td>{item.name}</td>
        <td>{item.date}</td>
        <td class="right-aligned">{formatCurrency(item.amount)}</td>
      </tr>
    {/each}
  </tbody>
...
### Best Practices
1. Use `right-aligned` class for numeric columns (amounts, quantities)
2. Include proper headers with descriptive text
3. Apply consistent padding across all cells
4. Use the footer section for totals and summaries
5. Consider adding zebra striping for larger tables with `data-table-row-alt` class
6. Implement responsive patterns for mobile viewing

### Table Actions
For action buttons within tables, use the following classes:
- `table-action-button`: Base styling for all table action buttons
- `table-action-button-primary`: Blue-styled action button
- `table-action-button-danger`: Red-styled action button for destructive actions
